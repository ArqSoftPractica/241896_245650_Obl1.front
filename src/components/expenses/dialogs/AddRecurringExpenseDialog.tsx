...
const AddRecurringExpenseDialog: React.FC<DialogProps> = ({ onClose, open, fetchExpenses }) => {
  ...
  const onAddExpenseHandler = (): void => {
    if (category && description && amount) {
      addRecurringTransaction({ categoryId: category, amount, description, type: 'expense' })
        .then(({ message }) => {
  ...
  return (
    ...
    >
      <Stack spacing={3}>
        <Typography textAlign="center">Recurring expenses will be added on the first day of each month.</Typography>
